...
function pet_state.force_emote(player_index, pet, emote)
    ...
    if pet and pet.valid then pet_visuals.emote(pet, emote) end
end
...
function pet_state.calculate_mood(player_index)
    ...
    if s.sadness >= MC.SAD then mood_table[#mood_table + 1] = "hurt" end
    ...
    -- Contented states.
    if s.hunger >= MC.FULL then mood_table[#mood_table + 1] = "happy" end
...
function pet_state.debug_dump(player_index)
    ...
    return string.format(
               "\n\thunger=%d\n\tloyalty=%d\n\tsadness=%d\n\tboredom=%d",
               s.hunger, s.loyalty, s.sadness, s.boredom)
end
